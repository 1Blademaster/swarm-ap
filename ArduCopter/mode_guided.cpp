...
void ModeGuided::takeoff_run()
{
    ...
    if (auto_takeoff.complete && !takeoff_complete)
    {
        ...
        // Check if swarm is active and transition to position control to not stay in takeoff mode
        AP_Swarm *swarm = AP::swarm();
        if (swarm != nullptr && swarm->have_target())
        {
            if (copter.g2.swarm.get_debug_level() >= 1)
            {
                hal.console->printf("Swarm (%d): Takeoff complete, switching to position control\n", (int)mavlink_system.sysid);
            }
            pos_control_start();
        }
    }
...
void ModeGuided::pos_control_run()
{
    ...
    // Check if swarm is active, if so override position target
    AP_Swarm *swarm = AP::swarm();
    if (swarm != nullptr && swarm->have_target())
    {
